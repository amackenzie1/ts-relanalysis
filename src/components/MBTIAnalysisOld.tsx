...
const MBTIAnalysis: React.FC<MBTIAnalysisProps> = ({ parsedData }) => {
  ...
  const performAnalysis = async () => {
    ...
    try {
      ...
      const sampleMessages = Object.keys(messagesPerUser).reduce(
        (acc, user) => {
  ...
  const calculateFinalPredictions = async (
    ...
    for (const [participant, mbtiList] of Object.entries(allPredictions)) {
      ...
      gptDescriptions[participant] = await getGPTDescription(
        mbtiType,
        participant,
        apiKey
      )
      ...
      const response = await axios.post(
        'https://api.openai.com/v1/chat/completions',
        {
          ...
          messages: [
            {
              role: 'user',
              content: `Provide a brief, funny but insightful description of the ${mbtiType} personality type for a person named ${username}, including key traits, strengths, and potential weaknesses. This should be humorous and slightly unhinged (lean a bit dark in humor but don't say anything too inappropriate) but also accurate. Use ${username}'s name in the output. Limit the response to 3-4 sentences.`,
            },
          ],
          temperature: 0.2,
        },
  ...
  const getLetterColor = (
    ...
    if (significance.includes(letter)) {
      return colors.uncertain
    } else if (letter === type[0] || letter === type[3]) {
      return colors.veryCertain
    } else if (letter === type[1]) {
      return colors.likelyCertain
    } else {
  ...
  const MBTIDisplay = ({ mbtiData }: { mbtiData: { [key: string]: { type: string; significance: string[] } } }) => (
    <div className="flex flex-col md:flex-row justify-center items-stretch gap-8 p-8 bg-gray-100 rounded-lg mb-8">
      {Object.entries(mbtiData).map(([name, data]) => (
        <div key={name} className="flex-1 min-w-[300px]">
            ...
            <div className="flex justify-center mb-4">
              {data.type.split('').map((letter, index) => (
                <span
                  ...
                  style={{ color: getLetterColor(letter, data.type, data.significance) }}
                >
                  {letter}
                </span>
              ))}
            ...
            {data.significance.length > 0 && (
              <div className="flex items-center justify-center text-gray-600">
                ...
                <span className="text-lg">
                  Uncertain: {data.significance.join('/')}
                </span>
              </div>
            )}
          </div>
  ...
  const traits: {
    ...
    ISTJ: {
      ...
      description:
        'ISTJs are dutiful, practical, and committed to traditions. They excel in creating and maintaining order but may struggle with change and emotional expression.',
    },
    ISFJ: {
      positives: ['Supportive', 'Reliable', 'Observant'],
      negatives: ['Overworked', 'Shy', 'Inflexible'],
      description:
        'ISFJs are caring, loyal, and detail-oriented. They thrive in nurturing roles but might have difficulty saying no or adapting to rapid changes.',
    },
    INFJ: {
      positives: ['Insightful', 'Idealistic', 'Creative'],
      negatives: ['Perfectionist', 'Sensitive', 'Burnout-prone'],
      description:
        'INFJs are idealistic, empathetic, and deeply intuitive. They have a strong sense of purpose but may struggle with perfectionism and feeling misunderstood.',
    },
    INTJ: {
      positives: ['Strategic', 'Independent', 'Innovative'],
      negatives: ['Arrogant', 'Overly critical', 'Socially detached'],
      description:
        'INTJs are analytical, strategic, and confident in their knowledge. They excel at complex problem-solving but may come across as aloof or overly critical.',
    },
    ISTP: {
      positives: ['Adaptable', 'Practical', 'Resourceful'],
      negatives: ['Unpredictable', 'Detached', 'Risk-prone'],
      description:
        'ISTPs are skilled troubleshooters with a knack for understanding how things work. They value freedom and may struggle with long-term commitments or emotional expression.',
    },
    ISFP: {
      positives: ['Artistic', 'Adaptable', 'Compassionate'],
      negatives: ['Overly competitive', 'Easily stressed', 'Conflict-avoidant'],
      description:
        'ISFPs are creative, sensitive, and in tune with their surroundings. They live in the moment but may have difficulty with long-term planning and confrontation.',
    },
    INFP: {
      positives: ['Idealistic', 'Empathetic', 'Creative'],
      negatives: ['Impractical', 'Self-isolating', 'Overly sensitive'],
      description:
        'INFPs are imaginative, empathetic, and driven by their values. They seek to understand themselves and others deeply but may struggle with criticism and practical matters.',
    },
    INTP: {
      positives: ['Analytical', 'Original', 'Open-minded'],
      negatives: ['Insensitive', 'Absent-minded', 'Procrastinating'],
      description:
        'INTPs are logical, creative problem-solvers with a thirst for knowledge. They excel in theoretical and analytical fields but may overlook emotional considerations and practical details.',
    },
    ESTP: {
      positives: ['Energetic', 'Practical', 'Adaptable'],
      negatives: ['Impatient', 'Risk-prone', 'Insensitive'],
      description:
        'ESTPs are action-oriented, adaptable, and enjoy living on the edge. They excel in crisis management but may struggle with long-term planning and emotional situations.',
    },
    ESFP: {
      positives: ['Enthusiastic', 'Spontaneous', 'Friendly'],
      negatives: ['Easily bored', 'Unfocused', 'Conflict-avoidant'],
      description:
        'ESFPs are vivacious, fun-loving, and enjoy being the center of attention. They live in the moment but may have difficulty with serious planning and following through on commitments.',
    },
    ENFP: {
      positives: ['Enthusiastic', 'Creative', 'Sociable'],
      negatives: ['Disorganized', 'Overly optimistic', 'Unfocused'],
      description:
        'ENFPs are charismatic, imaginative, and driven by their enthusiasm. They excel at generating ideas but may struggle with follow-through and practical details.',
    },
    ENTP: {
      positives: ['Innovative', 'Adaptable', 'Charismatic'],
      negatives: ['Argumentative', 'Insensitive', 'Unfocused'],
      description:
        "ENTPs are quick-thinking, charismatic idea generators. They thrive on intellectual challenges but may neglect practical matters and others' feelings in pursuit of their ideas.",
    },
    ESTJ: {
      positives: ['Organized', 'Practical', 'Reliable'],
      negatives: ['Inflexible', 'Judgmental', 'Stubborn'],
      description:
        "ESTJs are efficient, structured, and excel at implementing systems. They are natural leaders but may struggle with change and considering others' feelings in decision-making.",
    },
    ESFJ: {
      positives: ['Cooperative', 'Loyal', 'Warm'],
      negatives: ['Needy', 'Inflexible', 'Self-sacrificing'],
      description:
        'ESFJs are caring, social, and value harmony. They excel in creating and maintaining order in their environment but may be overly sensitive to criticism and neglect their own needs.',
    },
    ENFJ: {
      positives: ['Charismatic', 'Empathetic', 'Inspiring'],
      negatives: ['Overly idealistic', 'Approval-seeking', 'Controlling'],
      description:
        'ENFJs are charismatic leaders with a strong sense of idealism. They excel at motivating others but may struggle with being overly selfless and sensitive to criticism.',
    },
    ENTJ: {
      positives: ['Strategic', 'Confident', 'Efficient'],
      negatives: ['Domineering', 'Impatient', 'Cold'],
      description:
        'ENTJs are natural leaders with a talent for strategy and efficiency. They excel in organizing and implementing long-term plans but may come across as overly blunt or insensitive.',
    },
  }
          ...
          ([participant, mbtiType]) => (
            <div key={participant} className="w-full md:w-[48%] mb-8">
              ...
              <p className="mb-2">
                ...
                {analysis.significances[participant] &&
                  analysis.significances[participant].length > 0 && (
                    <span className="text-sm text-gray-600">
                      {' '}
                      (between{' '}
                      {analysis.significances[participant].join(' and ')})
                    </span>
                  )}
              </p>
              ...
              {/* Commented out GPT-generated profile
